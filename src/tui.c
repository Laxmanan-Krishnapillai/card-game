...
static void render_terminal(const Game *g)
{
    ...
    int rows = 0; for(int i=0;i<7;++i){ int h = engine_tableau_height(g,i); if(h>rows) rows = h; }
    if(rows < 4) rows = 4;                        /* room for F1‒F4 */
    ...
    for(int r=0; r<rows; ++r){
        ...
        if(r < 4){
            ...
            if(f) printf("%c%c", engine_rank_ch(f->rank), engine_suit_ch(f->suit));
            else printf("[]");
            printf(" F%d", r+1);
        }
        putchar('\n');
...
int main(void)
{
    ...
    while(1){
        if(g.phase == STARTUP) printf("INPUT > ");
        if(!fgets(buf, sizeof buf, stdin)) break;
        buf[strcspn(buf, "\n")] = '\0';
        if(!*buf) continue;                       /* ignore blank line */

        engine_execute(&g, buf);
        if(!strcmp(buf, "QQ")) break;            /* exit after processing */
